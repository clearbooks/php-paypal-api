<?php

namespace Paypal\Api;

include_once('AbstractRequestType.php');

class SetMobileCheckoutRequestType extends AbstractRequestType
{

    /**
     * @var SetMobileCheckoutRequestDetailsType $SetMobileCheckoutRequestDetails
     * @access public
     */
    public $SetMobileCheckoutRequestDetails = null;

    /**
     * @param DetailLevelCodeType[] $DetailLevel
     * @param string $ErrorLanguage
     * @param string $Version
     * @param string $any
     * @param SetMobileCheckoutRequestDetailsType $SetMobileCheckoutRequestDetails
     * @access public
     */
    public function __construct($DetailLevel, $ErrorLanguage, $Version, $any, $SetMobileCheckoutRequestDetails)
    {
      parent::__construct($DetailLevel, $ErrorLanguage, $Version, $any);
      $this->SetMobileCheckoutRequestDetails = $SetMobileCheckoutRequestDetails;
    }

}
